...
WsjcppYamlPlaceInFile::WsjcppYamlPlaceInFile(const std::string &sFilename, int nNumberOfLine,
...
std::string WsjcppYamlPlaceInFile::getFilename() const { return m_sFilename; }

void WsjcppYamlPlaceInFile::setFilename(const std::string &sFilename) { m_sFilename = sFilename; }

int WsjcppYamlPlaceInFile::getNumberOfLine() const { return m_nNumberOfLine; }

void WsjcppYamlPlaceInFile::setNumberOfLine(int nNumberOfLine) { m_nNumberOfLine = nNumberOfLine; }

std::string WsjcppYamlPlaceInFile::getLine() const { return m_sLine; }

void WsjcppYamlPlaceInFile::setLine(const std::string &sLine) { m_sLine = sLine; }
...
WsjcppYamlNode::WsjcppYamlNode(WsjcppYamlNode *pParent, IWsjcppYamlLog *pLog, const WsjcppYamlPlaceInFile &placeInFile,
...
WsjcppYamlNode *WsjcppYamlNode::getParent() { return m_pParent; }

// ---------------------------------------------------------------------

WsjcppYamlPlaceInFile WsjcppYamlNode::getPlaceInFile() { return m_placeInFile; }
...
void WsjcppYamlNode::setComment(const std::string &sComment) { m_sComment = sComment; }

// ---------------------------------------------------------------------

std::string WsjcppYamlNode::getComment() { return m_sComment; }
...
std::string WsjcppYamlNode::getName() { return m_sName; }

// ---------------------------------------------------------------------

WsjcppYamlQuotes WsjcppYamlNode::getNameQuotes() { return m_nNameQuotes; }

// ---------------------------------------------------------------------

bool WsjcppYamlNode::isEmpty() { return m_nItemType == WSJCPP_YAML_NODE_EMPTY; }
...
bool WsjcppYamlNode::isUndefined() { return m_nItemType == WSJCPP_YAML_NODE_UNDEFINED; }
...
bool WsjcppYamlNode::isMap() { return m_nItemType == WSJCPP_YAML_NODE_MAP; }
...
bool WsjcppYamlNode::setElement(const std::string &sName, WsjcppYamlNode *pItem) {
  ...
  if (m_nItemType != WSJCPP_YAML_NODE_MAP) {
    throw std::runtime_error(TAG + ": setElement, Element must be 'map' for " +
                             pItem->getPlaceInFile().getForLogFormat());
  }
  ...
  if (this->hasElement(sName)) { // TODO remove previous element
    throw std::runtime_error(TAG + ": setElement: Current map '" + this->getName() +
                             ...
                             ") "
                             "already has element with this name: '" +
                             sName + "'");
  }
...
bool WsjcppYamlNode::setElementValue(const std::string &sName, const std::string &sValue, WsjcppYamlQuotes nNameQuotes,
  ...
  if (m_nItemType != WSJCPP_YAML_NODE_MAP) {
    throw std::runtime_error(TAG + ": setElement, Element must be 'map' for " +
                             this->getPlaceInFile().getForLogFormat());
  }
...
bool WsjcppYamlNode::createElementMap(const std::string &sName, WsjcppYamlQuotes nNameQuotes) {
  if (m_nItemType != WSJCPP_YAML_NODE_MAP) {
    throw std::runtime_error(TAG + ": createElementMap, Element must be 'map' for " +
                             this->getPlaceInFile().getForLogFormat());
  }
  if (this->hasElement(sName)) {
...
WsjcppYamlNode *WsjcppYamlNode::createElementMap() {
  if (m_nItemType != WSJCPP_YAML_NODE_ARRAY) {
    throw std::runtime_error(TAG + ": createElementMap, Element must be 'array' for " +
                             this->getPlaceInFile().getForLogFormat());
  }
  WsjcppYamlPlaceInFile pl;
  WsjcppYamlNode *pNewItem = new WsjcppYamlNode(this, m_pLog, pl, WSJCPP_YAML_NODE_MAP);
...
bool WsjcppYamlNode::createElementArray(const std::string &sName, WsjcppYamlQuotes nNameQuotes) {
  if (m_nItemType != WSJCPP_YAML_NODE_MAP) {
    throw std::runtime_error(TAG + ": createElementArray, Element must be 'map' for " +
                             this->getPlaceInFile().getForLogFormat());
  }
  if (this->hasElement(sName)) {
    return false;
...
bool WsjcppYamlNode::isArray() { return m_nItemType == WSJCPP_YAML_NODE_ARRAY; }
...
WsjcppYamlNode *WsjcppYamlNode::getElement(int i) {
  ...
  if (pItem == nullptr) {
    throw std::runtime_error(TAG + ": getElement(" + std::to_string(i) + "), Out of range in array for '" +
                             this->getPlaceInFile().getLine() + "'");
  }
  return pItem;
}
...
bool WsjcppYamlNode::appendElement(WsjcppYamlNode *pNode) {
  ...
  if (m_nItemType != WSJCPP_YAML_NODE_ARRAY) {
    throw std::runtime_error(TAG +
                             ...
                             "\n"
                             "    line=" +
                             std::to_string(pNode->getNumberOfLine()) +
                             ")\n"
...
bool WsjcppYamlNode::isValue() { return m_nItemType == WSJCPP_YAML_NODE_VALUE; }
...
WsjcppYamlQuotes WsjcppYamlNode::getValueQuotes() { return m_nValueQuotes; }
    ...
    if (m_vObjects.size() > 0) {
      ...
      for (int i = 0; i < m_vObjects.size(); i++) {
        ...
        if (pNode->isEmpty()) {
          ...
          sRet += sIndent + pNode->getStringNodeLastIndent() + pNode->getSerializedName() + ":";
          if (pNode->hasObjects()) {
            sRet += "\n" + pNode->toString();
            removeLastCharNewLine(sRet);
          }
          ...
          sRet += sIndent + pNode->getStringNodeLastIndent() + pNode->getSerializedName() + ":" + sVal;
        }
        sRet += "\n";
      }
      removeLastCharNewLine(sRet);
...
std::string WsjcppYamlNode::getForLogFormat() { return m_placeInFile.getForLogFormat(); }

// ---------------------------------------------------------------------

int WsjcppYamlNode::getNodeLastIndent() { return m_nNodeDiffIndent; }

// ---------------------------------------------------------------------

std::string WsjcppYamlNode::getStringNodeLastIndent() { return m_sNodeDiffIndent; }
...
int WsjcppYamlNode::getNodeIndent() { return m_nNodeIndent; }

// ---------------------------------------------------------------------

int WsjcppYamlNode::getNumberOfLine() const { return m_placeInFile.getNumberOfLine(); }

void WsjcppYamlNode::setNumberOfLine(int nNumberOfLine) { m_placeInFile.setNumberOfLine(nNumberOfLine); }

void WsjcppYamlNode::throw_error(const std::string &sError) { throw std::runtime_error(sError.c_str()); }
...
bool WsjcppYamlNode::hasObjects() { return m_vObjects.size() > 0; }
...
WsjcppYamlParsebleLine::WsjcppYamlParsebleLine() : WsjcppYamlParsebleLine(-1) {}

// ---------------------------------------------------------------------

int WsjcppYamlParsebleLine::getLineNumber() { return m_nLineNumber; }

// ---------------------------------------------------------------------

std::string WsjcppYamlParsebleLine::getPrefix() { return m_sPrefix; }

// ---------------------------------------------------------------------

int WsjcppYamlParsebleLine::getIndent() { return m_sPrefix.length(); }

// ---------------------------------------------------------------------

bool WsjcppYamlParsebleLine::isArrayItem() { return m_bArrayItem; }

// ---------------------------------------------------------------------

std::string WsjcppYamlParsebleLine::getComment() { return m_sComment; }

// ---------------------------------------------------------------------

bool WsjcppYamlParsebleLine::hasComment() { return m_bHasComment; }

// ---------------------------------------------------------------------

std::string WsjcppYamlParsebleLine::getName() { return m_sTagName; }

// ---------------------------------------------------------------------

WsjcppYamlQuotes WsjcppYamlParsebleLine::getNameQuotes() { return m_nNameQuotes; }

// ---------------------------------------------------------------------

bool WsjcppYamlParsebleLine::isEmptyName() { return m_sTagName.length() == 0; }

// ---------------------------------------------------------------------

std::string WsjcppYamlParsebleLine::getValue() { return m_sValue; }

// ---------------------------------------------------------------------

WsjcppYamlQuotes WsjcppYamlParsebleLine::getValueQuotes() { return m_nValueQuotes; }

// ---------------------------------------------------------------------

bool WsjcppYamlParsebleLine::isEmptyValue() { return m_sValue.length() == 0; }

// ---------------------------------------------------------------------

bool WsjcppYamlParsebleLine::isEmptyLine() { return m_bEmptyLine; }
...
bool WsjcppYamlParsebleLine::parseLine(const std::string &sLine, std::string &sError) {
  ...
  for (int i = 0; i < sLine.length(); i++) {
    ...
    if ((c == ' ' || c == '\t') && state == WSJCPP_YAML_PARSER_LINE_STATE_NO) {
      m_sPrefix += c;
               ...
               (state == WSJCPP_YAML_PARSER_LINE_STATE_NO || state == WSJCPP_YAML_PARSER_LINE_STATE_VALUE)) {
    ...
    } else if (state == WSJCPP_YAML_PARSER_LINE_STATE_STRING_DOUBLE_QUOTES ||
               state == WSJCPP_YAML_PARSER_LINE_STATE_STRING_SINGLE_QUOTES) {
      m_sValue += c;
  ...
  if (state == WSJCPP_YAML_PARSER_LINE_STATE_STRING_DOUBLE_QUOTES ||
      state == WSJCPP_YAML_PARSER_LINE_STATE_STRING_SINGLE_QUOTES ||
      state == WSJCPP_YAML_PARSER_LINE_STATE_ESCAPING_DOUBLE_QUOTES ||
      state == WSJCPP_YAML_PARSER_LINE_STATE_ESCAPING_SINGLE_QUOTES) {
...
WsjcppYamlCursor::WsjcppYamlCursor() : WsjcppYamlCursor(nullptr) {
  // nothing
}
...
bool WsjcppYamlCursor::isNull() const { return m_pCurrentNode == nullptr; }

// ---------------------------------------------------------------------

bool WsjcppYamlCursor::isUndefined() const { return m_pCurrentNode != nullptr && m_pCurrentNode->isUndefined(); }

// ---------------------------------------------------------------------

bool WsjcppYamlCursor::isValue() const { return m_pCurrentNode != nullptr && m_pCurrentNode->isValue(); }

// ---------------------------------------------------------------------

bool WsjcppYamlCursor::isArray() const { return m_pCurrentNode != nullptr && m_pCurrentNode->isArray(); }
...
bool WsjcppYamlCursor::isMap() const { return m_pCurrentNode != nullptr && m_pCurrentNode->isMap(); }
...
std::string WsjcppYamlCursor::comment() { return m_pCurrentNode != nullptr ? m_pCurrentNode->getComment() : ""; }
...
std::string WsjcppYamlCursor::valStr() const { return m_pCurrentNode != nullptr ? m_pCurrentNode->getValue() : ""; }
...
int WsjcppYamlCursor::valInt() const {
  if (m_pCurrentNode != nullptr) {
    ...
    if (std::to_string(nValue) != sValue) {
      throw std::runtime_error(TAG + ": valInt, Element must be int but have a string" +
                               m_pCurrentNode->getForLogFormat());
    }
    return nValue;
  }
  return 0;
...
bool WsjcppYamlCursor::valBool() const {
  if (m_pCurrentNode != nullptr) {
    ...
    if (sValue == "yes" || sValue == "true") {
      ...
      throw std::runtime_error(TAG +
                               ": valBool, Element must be bool expected with ignore case like"
                               " 'yes', 'no', 'true', 'false' for " +
                               m_pCurrentNode->getForLogFormat());
    }
  }
  return false;
...
WsjcppYamlNode *WsjcppYamlCursor::node() { return m_pCurrentNode; }
...
WsjcppYamlCursor &WsjcppYamlCursor::operator=(const char *sVal) { return this->val(std::string(sVal)); }

WsjcppYamlCursor &WsjcppYamlCursor::operator=(const std::string &sVal) { return this->val(sVal); }

WsjcppYamlCursor &WsjcppYamlCursor::operator=(const int &nVal) { return this->val(nVal); }

WsjcppYamlCursor &WsjcppYamlCursor::operator=(const bool &bVal) { return this->val(bVal); }
...
WsjcppYaml::~WsjcppYaml() { delete m_pRoot; }

void WsjcppYaml::setLogger(IWsjcppYamlLog *pLog) { m_pLog = pLog; }
...
WsjcppYamlNode *WsjcppYaml::getRoot() { return m_pRoot; }

// ---------------------------------------------------------------------

WsjcppYamlCursor WsjcppYaml::getCursor() const { return WsjcppYamlCursor(m_pRoot); }

// ---------------------------------------------------------------------

WsjcppYamlCursor WsjcppYaml::operator[](int idx) const { return this->getCursor()[idx]; }

// ---------------------------------------------------------------------

WsjcppYamlCursor WsjcppYaml::operator[](const std::string &sName) const { return this->getCursor()[sName]; }
...
bool WsjcppYaml::readTextFile(const std::string &sFilename, std::string &sOutputContent, std::string &sError) {
  ...
  sOutputContent = std::string((std::istreambuf_iterator<char>(f)), std::istreambuf_iterator<char>());
...
bool WsjcppYaml::writeFile(const std::string &sFilename, const std::string &sContent) {
  ...
  if (!f) {
    std::cerr << "WsjcppYaml::writeFile"
              << "Could not create file to write '" << sFilename << "'" << std::endl;
    return false;
  }
  f << sContent << std::endl;
  f.close();
  ...
  for (int nLine = 0; nLine < vLines.size(); nLine++) {
    ...
    if (m_parseLine.isEmptyLine()) {
      if (m_pParseCurrentParentNode != nullptr) {
        ...
        if (m_pParseCurrentParentNode->isArray() || m_pParseCurrentParentNode->isMap() ||
            m_pParseCurrentParentNode->isUndefined()) {
          // WsjcppLog::warn(TAG, "array, map or undefined");
          WsjcppYamlNode *pNode =
              new WsjcppYamlNode(m_pParseCurrentParentNode, this, m_parsePlaceInFile, WSJCPP_YAML_NODE_EMPTY);
          pNode->setNodeIndents(m_vStackDiffNodeIndents);
          m_pParseCurrentParentNode->appendElement(pNode);
        } else if (m_pParseCurrentParentNode->getParent() != nullptr &&
                   (m_pParseCurrentParentNode->getParent()->isArray() ||
                    m_pParseCurrentParentNode->getParent()->isMap())) {
          // WsjcppLog::warn(TAG, "parent exists and parent map or array");
          WsjcppYamlNode *pNode = new WsjcppYamlNode(m_pParseCurrentParentNode->getParent(), this, m_parsePlaceInFile,
...
void WsjcppYaml::process_hasName_emptyValue_noArrayItem() {
  ...
  WsjcppYamlNode *pNode =
      new WsjcppYamlNode(m_pParseCurrentParentNode, this, m_parsePlaceInFile, WSJCPP_YAML_NODE_UNDEFINED);
  if (m_parseLine.getValueQuotes() != WSJCPP_YAML_QUOTES_NONE) {
    // std::cout << "pNode->doValue() for '" << m_parseLine.getName() << "' value: '" << m_parseLine.getValue() << "' "
    // << std::endl;
    pNode->doValue();
...
void WsjcppYaml::process_hasName_hasValue_arrayItem() {
  ...
  WsjcppYamlNode *pMapItem =
      new WsjcppYamlNode(m_pParseCurrentParentNode, this, m_parsePlaceInFile, WSJCPP_YAML_NODE_MAP);
  ...
  WsjcppYamlNode *pNode =
      new WsjcppYamlNode(m_pParseCurrentParentNode, this, m_parsePlaceInFile, WSJCPP_YAML_NODE_VALUE);
  pNode->setComment(m_parseLine.getComment());
  pNode->setValue(m_parseLine.getValue(), m_parseLine.getValueQuotes());
  pNode->setName(m_parseLine.getName(), m_parseLine.getNameQuotes());
...
void WsjcppYaml::logUnknownParseLine() {
  this->warn(TAG, "\n"
                  ...
                  "    desc: \"unknown_line\"\n"
                  "    line_number: " +
                      std::to_string(m_pParseCurrentParentNode->getPlaceInFile().getNumberOfLine()) +
                      "\n"
                      "    line: \"" +
                      m_parsePlaceInFile.getLine() +
                      "\"\n"
                      "    indent: " +
                      std::to_string(m_nParseCurrentIndent) +
                      "\n"
                      "    filename: \"" +
